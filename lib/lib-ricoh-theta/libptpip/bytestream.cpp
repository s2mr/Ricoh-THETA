...
namespace PTP {
    ...
    namespace String {
        void to_unicodez_str(PTP::ByteStream& stream, string& ret)
        {
            ...
            throw RUNTIME_ERROR("Not enough stream");
        }
        ...
        static void iconv(const string& src, string& ret, const char* from_code, const char* to_code)
        {
            ...
            if(iv==(iconv_t)-1) throw RUNTIME_ERROR("iconv_open() failed.", errno);
            ...
            if(out_len==(size_t)-1) throw RUNTIME_ERROR("iconv() failed.", errno, src.c_str(), src.size());
